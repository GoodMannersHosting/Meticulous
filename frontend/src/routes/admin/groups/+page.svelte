<script lang="ts">
	import { Users, Plus, Search, MoreVertical, UserCog, Trash2, Edit, UserPlus, X, Link2 } from 'lucide-svelte';
	import { apiMethods } from '$lib/api';
	import type { AdminGroup, GroupMember, AdminUser, AuthProviderResponse, GroupMappingResponse } from '$lib/api/types';
	...
	async function openAddMemberModal() {
		...
		try {
			...
			availableUsers = response.data.filter(u => !existingMemberIds.has(u.id));
		} catch (e) {
