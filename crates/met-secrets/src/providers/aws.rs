...
impl Default for AwsSecretsConfig {
    fn default() -> Self {
        Self {
            region: None, endpoint_url: None, access_key_id: None,
            secret_access_key: None, oidc_token: None, role_arn: None,
            timeout_secs: 30,
        }
    }
}
...
impl SecretsProvider for AwsSecretsProvider {
    async fn get_secret(&self, path: &str) -> Result<SecretValue> {
        ...
        let aws_resp: AwsSecretResponse = resp.json().await
            .map_err(|e| SecretsError::ProviderError {
                provider: "aws_sm".into(),
                message: format!("failed to parse response: {e}"),
            })?;

        let value = aws_resp.secret_string.ok_or_else(|| SecretsError::InvalidFormat {
            message: "SecretString is null (binary secrets not supported)".into(),
        })?;

        Ok(SecretValue::with_metadata(value, SecretMetadata {
    ...
    async fn list_secrets(&self, prefix: &str) -> Result<Vec<String>> {
        ...
        let body: serde_json::Value = resp.json().await
            .map_err(|e| SecretsError::ProviderError { provider: "aws_sm".into(), message: e.to_string() })?;
        let names = body["SecretList"].as_array()
            .map(|arr| arr.iter().filter_map(|v| v["Name"].as_str().map(String::from)).collect())
            .unwrap_or_default();
        Ok(names)
    }

    fn provider_type(&self) -> ProviderType { ProviderType::AwsSecretsManager }
...
mod tests {
    ...
    async fn test_provider_type() {
        let provider = AwsSecretsProvider::new(AwsSecretsConfig::default()).await.unwrap();
