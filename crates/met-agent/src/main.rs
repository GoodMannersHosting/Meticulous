...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Reuse one SIGTERM `Signal` for the whole process lifetime. A prior approach called a helper that ran
    // `unix_signal(terminate)` on every loop iteration; dropping the old listener can lose SIGTERM while
    // other branches (heartbeat actions) complete.
    #[cfg(unix)]
